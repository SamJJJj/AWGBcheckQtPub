...
{
    ui.setupUi(this);
    QRegExp regExp("\\d{1,20}");
    QRegExp rxIp("[0-9\\.]{1,15}");
    QRegExp url(".{1,1024}");
    ...
    ui.localId->setValidator(new QRegExpValidator(regExp, ui.localId));
    ui.localArea->setValidator(new QRegExpValidator(regExp, ui.localArea));
    ...
    ui.LineEdit_10->setValidator(new QRegExpValidator(url, ui.LineEdit_10));
