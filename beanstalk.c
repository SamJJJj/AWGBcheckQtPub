...
int bs_connect(const char *host, int port) {
    ...
    WSADATA wsaData;
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) return BS_STATUS_FAIL;
...
int bs_connect_with_timeout(const char *host, int port, float secs)
{
    ...
    if (res < 0) {
        if (errno == EINPROGRESS) {
            ...
            int ret = WSAPoll(&pfd, 1, (int)(secs*1000));
            if (ret > 0) {
...
size_t bs_send_message(int fd, char *message, size_t size)
{
    ...
    return send(fd, message, size, bs_poll ? 0x40|MSG_NOSIGNAL : MSG_NOSIGNAL);
}
